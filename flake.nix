...
# [`nix/flake.cc::CmdFlakeCheck::run`](https://github.com/NixOS/nix/blob/master/src/nix/flake.cc#L502).
{
  ...
  outputs =
    ...
    let
      ...
      systemPkgsYubikey = eachSystem (
        system: inputs.nixpkgs-yubikey.legacyPackages.${system}
      );
    ...
    in
    {
      packages = eachSystem (
        ...
        in
        {
          ...
          nixos-iso =
            self.nixosConfigurations.isoGraphicalInstaller.system.build.isoImage;
      ...
      homeConfigurations."phliptop-nitro" =
        inputs.home-manager.lib.homeManagerConfiguration
          rec {
            pkgs = systemPkgs."x86_64-linux";
            lib = pkgs.lib;
      ...
      homeConfigurations."phliptop-mbp" =
        inputs.home-manager.lib.homeManagerConfiguration
          rec {
