...
in
{
  ...
  config.systemd.services.xremap = lib.mkIf cfg.enable {
    ...
    serviceConfig = {
      ExecStart =
        let
          xremap = lib.getExe cfg.package;
          args = builtins.concatStringsSep " " cfg.extraArgs;
          configFile =
            if builtins.typeOf cfg.config == "path" then
              cfg.config
            else
              pkgs.writeText "config.yml" cfg.config;
        in
        "${xremap} ${args} ${configFile}";
      Nice = -20;
      OOMScoreAdjust = -1000;
